...
enron data with some adjustments: job positions etc. data.
...
It solves this problem using a function.
hv- and heprops will change the structure of the hierarchical networks.
Provide labels for nodes in the graphs (i.e., job positions).
...
We will use increasingly rigorous models.

We'll also initialize a couple of empty dictionaries that we'll use to collect information about our models once they've been fit, and then compare them. The first will store each model's Minimum Description Length (MDL), the second will store homogeneity scores.
Data are employees, that are nodes in a network. They send each other emails (i.e., edges) in a network (i.e., organization). Our theory is that the email relationships in the organization are related to the role that individuals have in the organization.
...
Base minimize description lenght nested blockmodel.
...
This visualization conveys a lot of information about the estimated network structure. Each node's job title is printed on the node (although you'll really have to squint to read it). You may notice that some blocks contain clusters of folks with the same or similar job titles (by looking closely at the labels). This makes some intuitive sense, especially when considered in terms of **positional network analysis** and the concept of **stochastic equivalence**. It's plausible, likely even, that the formal roles people hold in the organization generate different relational patterns, however noisy, and that these patterns would be picked up by the blockmodel.
...
Show first ten vertexes. Each one has a position category and block ID.
...
We don't have anything to compare this score with, which we would need to understands if this model works well.
...
plt.title("ECDF of edge weights\nin the Enron email network\n", loc="left")
...
We have a heavvy tail! The majority of the network (about 90%) has a small number of edges.
The most appropriate way to model it would be a negative binomial, that are not in graph-tool. But we can use a Poisson.
...
Distribution and edge weights are inside a list in graph-tool.
Poisson arguments allow to have a prior on the probability distribution.
Then, get the newest partition distribution that the model thinks it has the lowest description lenght with this additional requirement of a Poisson distribution.
...
```
At the lowest level of the network, it is similar as before. Moving up the hierarchy, things change a bit.
We want to know if this is better than the model before.

We'll also add the new block assignments to the `vertex_data` dataframe we created above.
Use a dictionary mapping vertex data in vertixID column.
...
This is not a fair comparison with a binary model, as things are very different.
...
Homogeneity has increased in the second model. More homogeneity within the blocks at level 0 than in the binary network --> smaller communities at the smallest level.
...
It computes the probability of each possible explanation for links between emails, and then sees which ones are most likely. The first two models pick the partition with the minimum description lenght. Now it checks for the maximum posterior distribution in the partition.
It takes the minimum distant lenght and selects it in this loop:
...
It is more comparable with model 3. 3 has lower description lenght than 2-->improvement! More efficient description of networks.
...
3 has a higher homogeneity than 2.
3 is the best model?

## Posterior Inference

Before, we only found the best partition.
But typically, we want to analyse the entire posterior of the distribution.
...
This has an improvement event with respect to model 3, that was already very good.
...
Probabilities are all quite high.
...
Some nodes are little pie charts whose fractions indicate the probability of assignment to the cluster. The vast majority of nodes have been confidently clustered in a group at level 0.
...
Look for different partitions in the cluster and compute probabilities. Map as competing explanations those with higher probability.
